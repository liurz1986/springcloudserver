package com.kedacom.apigateway.filter;
...
import com.kedacom.apigateway.Service.LogService;
import com.kedacom.apigateway.Service.RedisService;
	...
	public Object run() {
		...
		if (StringUtils.isEmpty(userId)) {
			...
			if (StringUtils.isEmpty(userId)) {
				...
				logInformaton.put("status", -1);
				logInformaton.put("message", "userId is empty");
				logService.saveLogs(logInformaton);
				logger.warn("userId is empty");
				return null;
			}
		}
		String user = redisService.get(userId);
		if (StringUtils.isEmpty(user)) {
			ctx.setSendZuulResponse(false);
			ctx.setResponseStatusCode(HttpStatus.UNAUTHORIZED.value());
			ctx.setResponseBody("用户不存在");
			logInformaton.put("status", -1);
			logInformaton.put("message", "用户不存在");
			logInformaton.put("user", userId);
			logService.saveLogs(logInformaton);
			logger.warn("用户不存在");
			return null;
		}
